...
type
  TDTM_A017ADPT = class(TDataModule)
    ...
    QRY_154CDESCRIULC: TStringField;
    QRY_154DAT_DEST: TStringField;
    ...
    procedure QRY_152BeforeInsert(DataSet: TDataSet);
  private
...
var
  DTM_A017ADPT: TDTM_A017ADPT;

implementation

uses A017ADPT, MPLLib;

{$R *.DFM}

procedure TDTM_A017ADPT.DTS_154DataChange(Sender: TObject; Field: TField);
begin
  with FRM_A017ADPT do
    begin
      DBD_DDTORIGEM.Text :=
...
procedure TDTM_A017ADPT.QRY_152AfterScroll(DataSet: TDataSet);
begin
  with FRM_A017ADPT do
    begin
...
procedure TDTM_A017ADPT.QRY_152AfterRefresh(DataSet: TDataSet);
begin
  QRY_152AfterScroll(DataSet);
end;

procedure TDTM_A017ADPT.QRY_152NNROPROTValidate(Sender: TField);
begin
...
//          if Sender.AsInteger <> nProtocolo then
            begin
              ...
              if QRY_CONSUL.IsEmpty then
                ...
                if (QRY_CONSUL.FieldByName('cemigtm').asString = 'N') and
                   (DED_CORIGEM.TExt <> 'PROTOCOLO')  then
                  begin
                  ...
                  if QRY_CONSUL.FieldByName('csituareq').asString = 'CANCELADO' then
                    ...
                    if QRY_CONSUL.FieldByName('ddtprot').AsDateTime >
                      ...
                      begin
                        if DED_CORIGEM.Text = 'PROTOCOLO' then
...
procedure TDTM_A017ADPT.QRY_152BeforeDelete(DataSet: TDataSet);
begin
  FRM_A017ADPT.ListProtAtua.Add(DataSet.FieldByName('nidcadreq').asString);
  if FRM_A017ADPT.ListProtocolo.IndexOf(DataSet.FieldByName('nnroprot').asString) <> -1 then
    FRM_A017ADPT.ListProtocolo.Delete(
    FRM_A017ADPT.ListProtocolo.IndexOf(DataSet.FieldByName('nnroprot').asString));
end;

procedure TDTM_A017ADPT.QRY_152BeforePost(DataSet: TDataSet);
begin
  if DataSet.State = dsInsert then
    begin
      FRM_A017ADPT.ListProtocolo.Add(DataSet.FieldByName('nnroprot').asString);
      FRM_A017ADPT.ListProtAtua.Add(DataSet.FieldByName('nidcadreq').asString);
    end;
end;

procedure TDTM_A017ADPT.QRY_152NNROPROTSetText(Sender: TField;
  const Text: String);
begin
  if Sender.Text <> '' then
...
procedure TDTM_A017ADPT.QRY_152BeforeInsert(DataSet: TDataSet);
begin
 Abort;
end;
